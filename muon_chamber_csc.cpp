...
// Implementation of member functions for cathode strip chamber (CSC) muon chamber derived class
...
int Muon_Chamber_CSC::s_csc_detection_count=0;

// Paramtersied constructor
Muon_Chamber_CSC::Muon_Chamber_CSC(const std::string &name, int component_count, double detection_efficiency, bool status)
  : Muon_Chamber(name, component_count, detection_efficiency, status)
{}

// Print function
void Muon_Chamber_CSC::print_info() const
{
  std::cout<<"Muon Chamber CSC: Name = "<<m_name
...
bool Muon_Chamber_CSC::detect_particle(const Particle &particle)
{
  ...
  // Detected
  ++s_csc_detection_count;
  std::cout<<"Muon detected by Muon Chamber CSC!"<<std::endl;
